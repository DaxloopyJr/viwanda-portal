...
<head>
    ...
    <title>@yield('title', 'Viwanda Portal') — {{ \App\Models\Setting::siteName() }}</title>
...
<div id="pageLoader">
    ...
    <div class="loader-text">{{ \App\Models\Setting::siteName() }}</div>
</div>
@auth
{{-- Header: brand + user (sticky) --}}
<header class="app-header d-flex align-items-center justify-content-between">
    <a class="navbar-brand d-flex align-items-center gap-2" href="{{ route('dashboard') }}">
        ...
        @endif
        <span>
            <span class="d-block brand-name">{{ \App\Models\Setting::siteName() }}</span>
            <span class="brand-slogan d-none d-md-block">{{ \App\Models\Setting::tagline() }}</span>
        </span>
    </a>
    <div class="dropdown">
        ...
        <ul class="dropdown-menu dropdown-menu-end">
            <li class="px-3 py-2">
                ...
                <div class="text-muted small">{{ auth()->user()->roles->pluck('name')->join(', ') ?: 'No role' }}</div>
                @if(auth()->user()->institution)<div class="text-muted small">{{ auth()->user()->institution->name }}</div>@endif
                @if(auth()->user()->department)<div class="text-muted small">MIT — {{ auth()->user()->department->name }}</div>@endif
            </li>
            <li><hr class="dropdown-divider"></li>
            <li><a class="dropdown-item {{ request()->routeIs('profile.*') ? 'active' : '' }}" href="{{ route('profile.show') }}"><i class="bi bi-person me-2"></i>Profile &amp; API Tokens</a></li>
            <li>
...
{{-- Nav menu bar (sticky, below the header) --}}
            ...
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                ...
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item {{ request()->routeIs('submissions.index') ? 'active' : '' }}" href="{{ route('submissions.index') }}"><i class="bi bi-list-ul me-2"></i>All Submissions</a></li>
                        @can('submissions.create')
                        <li><a class="dropdown-item {{ request()->routeIs('submissions.create') ? 'active' : '' }}" href="{{ route('submissions.create') }}"><i class="bi bi-plus-lg me-2"></i>New Submission</a></li>
                        @endcan
                    </ul>
                </li>
                @endcan
                ...
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item {{ request()->routeIs('datasets.index') ? 'active' : '' }}" href="{{ route('datasets.index') }}"><i class="bi bi-list-ul me-2"></i>All Datasets</a></li>
                        @canany(['datasets.manage', 'datasets.manage-own'])
                        <li><a class="dropdown-item {{ request()->routeIs('datasets.create') ? 'active' : '' }}" href="{{ route('datasets.create') }}"><i class="bi bi-plus-lg me-2"></i>New Dataset</a></li>
                        @endcanany
                        @canany(['periods.manage', 'periods.manage-own'])
                        <li><a class="dropdown-item {{ request()->routeIs('periods.*') ? 'active' : '' }}" href="{{ route('periods.index') }}"><i class="bi bi-calendar3 me-2"></i>Submission Periods</a></li>
                        @endcanany
                        @canany(['consumers.manage', 'consumers.manage-own'])
                        <li><a class="dropdown-item {{ request()->routeIs('consumers.*') ? 'active' : '' }}" href="{{ route('consumers.index') }}"><i class="bi bi-people me-2"></i>Data Consumers</a></li>
                        @endcanany
                    </ul>
                </li>
                ...
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item {{ request()->routeIs('reports.thematic*') ? 'active' : '' }}" href="{{ route('reports.thematic') }}"><i class="bi bi-journal-richtext me-2"></i>Management Reports (R1–R14)</a></li>
                        <li><a class="dropdown-item {{ request()->routeIs('reports.consolidated') ? 'active' : '' }}" href="{{ route('reports.consolidated') }}"><i class="bi bi-pie-chart me-2"></i>Consolidated Report</a></li>
                        <li><a class="dropdown-item {{ request()->routeIs('reports.compliance') ? 'active' : '' }}" href="{{ route('reports.compliance') }}"><i class="bi bi-check2-square me-2"></i>Submission Compliance</a></li>
                        @can('reports.export')
                        <li><a class="dropdown-item" href="{{ route('reports.export') }}"><i class="bi bi-download me-2"></i>Export CSV</a></li>
                        @endcan
                    </ul>
                </li>
                @endcan

                @canany(['institutions.manage', 'departments.manage', 'users.manage', 'users.manage-own', 'roles.manage', 'audit.view', 'settings.manage'])
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li><a class="dropdown-item {{ request()->routeIs('institutions.*') ? 'active' : '' }}" href="{{ route('institutions.index') }}"><i class="bi bi-bank me-2"></i>Institutions</a></li>
                        @endcan
                        @can('departments.manage')
                        <li><a class="dropdown-item {{ request()->routeIs('departments.*') ? 'active' : '' }}" href="{{ route('departments.index') }}"><i class="bi bi-diagram-3 me-2"></i>Ministry Departments</a></li>
                        @endcan
                        @canany(['users.manage', 'users.manage-own'])
                        <li><a class="dropdown-item {{ request()->routeIs('users.*') ? 'active' : '' }}" href="{{ route('users.index') }}"><i class="bi bi-people me-2"></i>Users</a></li>
                        @endcanany
                        @can('roles.manage')
                        <li><a class="dropdown-item {{ request()->routeIs('roles.*') ? 'active' : '' }}" href="{{ route('roles.index') }}"><i class="bi bi-shield-lock me-2"></i>Roles &amp; Permissions</a></li>
                        @endcan
                        @can('settings.manage')
                        <li><a class="dropdown-item {{ request()->routeIs('settings.*') ? 'active' : '' }}" href="{{ route('settings.edit') }}"><i class="bi bi-sliders me-2"></i>Portal Settings</a></li>
                        @endcan
                        @can('audit.view')
                        <li><a class="dropdown-item {{ request()->routeIs('audit.*') ? 'active' : '' }}" href="{{ route('audit.index') }}"><i class="bi bi-shield-check me-2"></i>Audit Trail</a></li>
                        @endcan
                    </ul>
                </li>
                @endcanany
...
<div class="container-fluid px-4">
    ...
    <footer class="text-center text-muted small py-3 border-top">{{ \App\Models\Setting::siteName() }} &copy; {{ date('Y') }}</footer>
</div>
