...
<head>
    ...
    <title>@yield('title') — {{ \App\Models\Setting::siteName() }}</title>
            ...
            <div class="card-body p-4 p-md-5">
                <div class="text-center mb-4">
                    @if(\App\Models\Setting::logoUrl())
                        <img src="{{ \App\Models\Setting::logoUrl() }}" class="auth-logo mb-3" alt="logo">
                    @else
                        <span class="auth-emblem mb-3"><i class="bi bi-bank"></i></span>
                    @endif
                    <h4 class="fw-bold auth-title mb-1">{{ \App\Models\Setting::siteName() }}</h4>
                    <p class="auth-sub small mb-0">{{ \App\Models\Setting::tagline() }}</p>
                </div>
                @if($errors->any())<div class="alert alert-danger py-2"><ul class="mb-0">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>@endif
                @yield('content')
            </div>
        </div>
        <p class="text-center small mt-3 mb-0" style="color:rgba(255,255,255,.75)">{{ __('Authorized users only. All activity is audited.') }}</p>
    </div>
</body>
</html>
